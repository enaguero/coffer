...
- **Onboarding**: `components/GettingStarted.tsx` renders a state-driven checklist on the Dashboard (accounts → first import → rule → budget → goal), derived from live queries — no stored wizard step; hidden when complete or dismissed (localStorage). The Import page supports a multi-file backfill queue (files advance pending → reviewing → done/skipped/failed, auto-previewing the next after each commit/discard) and inline account quick-create (auto-opens for users with zero accounts).
